...
(function ($) {
     ...
     * 错误处理
     */
    function handleError(jqXHP, textStatus, errorThrown) {
        if (! window.console || !window.console.trace) {
            return;
        }
        if (jqXHP.status == 499) {
            var statusText = jqXHP.statusText;
            if (statusText != null && statusText != undefined && statusText != '' && statusText.indexOf('$') > 0) {
                var startUrl = statusText.indexOf('$') + 1;
                var loginUrl = statusText.substring(startUrl);
                window.location.href = loginUrl;
            } else {
                epd.notify.error('登录信息失效，请重新登录', "错误消息");
            }
        } else {
            var msg = "请求错误！";
            switch (textStatus) {
