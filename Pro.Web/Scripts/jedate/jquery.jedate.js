...
; (function (root, factory) {
    ...
    jet.getYM = function (y, m, n) {
        var nd = new Date(y, m - n);
        var month = nd.setMonth(m - 1 + n);
        return {
            y: nd.getFullYear(),
            m: nd.getMonth() + 1
        };
    ...
    jet.getPrevMonth = function (y, m, n) {
        var nd = jet.getYM(y, m, 0 - (n || 1));
        return nd;
    }
    //获取下个月
    jet.getNextMonth = function (y, m, n) {
        var nd = jet.getYM(y, m, n || 1);
        return nd;
    }
    //补齐数位
    ...
    jedfn.setHtml = function (opts) {
        ...
        //是否显示清除按钮
        jet.isBool(opts.isClear) ? "" : jet.isShow(boxCell.find(".jedatebot .jedateclear"), true);
        //是否显示今天按钮
        if (!isYYMM) {
            jet.isBool(opts.isToday) ? "" : jet.isShow(boxCell.find(".jedatebot .jedatetodaymonth"), true);
        };
            ...
            !ishhmmss && $.each([yPre, yNext], function (i, cls) {
                cls.on("click", function (ev) {
                    ...
                    var year = parseInt(jedateyear.attr("year")),
                        ...
                        PrevYM = jet.getPrevMonth(pnYear, month),
                        NextYM = jet.getNextMonth(pnYear, month);
                    PrevYM.y = pnYear;
                    NextYM.y = pnYear;
